...
## Codex — canonical docs (read first)
The source of truth for what Cursory IS and the rules that keep it coherent lives in `docs/`:
- **`docs/BIBLE.md`** (L0) — the one sentence, what it is NOT, architecture canon (nouns/verbs),
  the Laws (`#CUR-LAW-n`, inheriting `../MindAttic.HouseRules.md`), verified state, frontier,
  quality bar, glossary. Cite stable anchors (`{#CUR-§N}`, `{#CUR-LAW-n}`), never line numbers.
- **`docs/USER_STORIES.md`** (L2) — `CUR-US-<Epic><n>` stories; every ✅ names its NUnit test.
- **`docs/AMENDMENTS.md`** (L1) — append-only; an amendment wins over the bible (`#CUR-A1`:
  Aether.Physics2D replaced sum-of-springs; switches/doors retired).
- **`docs/rfc/`** — design notes that graduate into the bible + stories.
- **`docs/BIBLE.digest.md`** — GENERATED by `tools/codex.ps1 digest`; never hand-edit. A
  SessionStart hook (`.claude/hooks/inject-digest.ps1`) injects it as authoritative context.

A fact lives in exactly one layer; link to it by id. After editing canon, run
`powershell -File tools/codex.ps1 digest` then `powershell -File tools/codex.ps1 doctor`
(doctor must pass). Mark a story ✅ only when a test/build proves it (HOUSE-LAW-8); else 🟡/⬜.
